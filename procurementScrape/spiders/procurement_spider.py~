...
from procurementScrape.items import Tender, Organisation, TenderBidder, TenderAgreement, TenderDocument, CPVCode, WhiteListObject, BlackListObject, Complaint
import os
import sys
import httplib2
import shutil
...
class ProcurementSpider(BaseSpider):
    ...
    def parseBlackListItem(self, response):
      ...
      nameDiv = hxs.select('//div[@class="txt_page"]//div[@class="name"]').extract()[0]
      dateDiv = hxs.select('//div[@class="txt_page"]//div[@class="indate"]').extract()[0]
      infoDivs = hxs.select('//div[@class="txt_page"]//div[@class="txt"]//p').extract()
      procurerDiv = infoDivs[2]
      tenderDiv = infoDivs[3]
      reasonDiv = infoDivs[6]
      ...
      startIndex = procurerDiv.find(">")
      startIndex = procurerDiv.find(":",startIndex)
      endIndex = procurerDiv.find(";",startIndex)
      procurer = procurerDiv[startIndex+1:endIndex]

      tenderNum = "NULL"
      startIndex = tenderDiv.find("<a")
      if startIndex == -1:
        tenderID = "NULL"
        startIndex = tenderDiv.find(">")
        startIndex = tenderDiv.find(";",startIndex)
        print "ID"
        print tenderDiv
        print "SINDEX :" + str(startIndex)
        endIndex = tenderDiv.find(";",startIndex+1)
        print "EINDEX :" + str(endIndex)
        tenderNum = tenderDiv[startIndex+1:endIndex]
      else:
        startIndex = tenderDiv.find(">",startIndex)
        endIndex = tenderDiv.find("</",startIndex)
        tenderID = tenderDiv[startIndex+1:endIndex]

      startIndex = reasonDiv.find(">")
      startIndex = reasonDiv.find(":",startIndex)
      endIndex = reasonDiv.find("<",startIndex)
      reason = reasonDiv[startIndex+1:endIndex]
    ...
    def parseWhiteListUrls(self, response):
      ...
      for link in whiteListLinks:
        ...
        request = Request(url,callback=self.parseWhiteListItem, cookies={"SPALITE":self.sessionCookie}, headers={"User-Agent":self.userAgent})
        yield request
      ...
      if nextPage <= final_page:
        ...
        request = Request(url,callback=self.parseWhiteListUrls, meta = metadata, cookies={"SPALITE":self.sessionCookie}, headers={"User-Agent":self.userAgent})
        yield request    
    ...
    def parseBlackListUrls(self, response):
      ...
      for link in blackListLinks:
        ...
        request = Request(url,callback=self.parseBlackListItem, cookies={"SPALITE":self.sessionCookie}, headers={"User-Agent":self.userAgent})
        yield request
      ...
      if nextPage <= final_page:
        ...
        request = Request(url,callback=self.parseBlackListUrls, meta = metadata, cookies={"SPALITE":self.sessionCookie}, headers={"User-Agent":self.userAgent})
        yield request 

    

    self.getDataByLabel( label, divs )
      count = 0
      dataIndex = -1
      for div in divs:
        if div.find(label) > -1:
    ...
    self.parseDispute(self, response)
      hxs = HtmlXPathSelector(response)
      divs = hxs.select('//div').extract()

      item = ComplaintObject()

      statusDiv = self.getDataByLabel(u"საჩივრის მიმდინარე სტატუსი")
      ...
      organizationDiv = self.getDataByLabel(u"მომჩივანი")
    ...
    self.parseDisputeLinks(self, response)
      ...
      for disputeLink in disputeLinks:
        ...
        request = Request(url,callback=self.parseDispute, cookies={"SPALITE":self.sessionCookie}, headers={"User-Agent":self.userAgent})
        yield request
    ...
    def parse(self, response):
      ...
      request = Request(url,callback=self.parseDisputeLinks, cookies={"SPALITE":self.sessionCookie}, headers={"User-Agent":self.userAgent})
      yield request
